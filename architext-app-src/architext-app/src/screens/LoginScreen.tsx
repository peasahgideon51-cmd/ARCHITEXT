    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: theme.pageBg }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <View style={styles.header}>
          ...
          <Text style={[styles.title, { color: theme.dark }]}>Welcome back</Text>
          <Text style={[styles.sub, { color: theme.muted }]}>Sign in to access your floor plans</Text>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: theme.cardBg, borderColor: theme.border }]}>
          ...
          <Input label="Password" value={password} onChangeText={setPassword} placeholder="••••••••" secureTextEntry containerStyle={styles.gap} />
          <Button label="Sign In" onPress={handleLogin} loading={loading} style={{ marginTop: 6 }} />
          <TouchableOpacity style={styles.forgotRow}>
            <Text style={[styles.forgotText, { color: theme.brown }]}>Forgot password?</Text>
          </TouchableOpacity>
        </View>

        <View style={styles.switchRow}>
          <Text style={[styles.switchText, { color: theme.muted }]}>Don't have an account? </Text>
          <TouchableOpacity onPress={onNavigateToSignUp}>
            <Text style={[styles.switchLink, { color: theme.brown }]}>Sign up</Text>
          </TouchableOpacity>
        </View>

        <TouchableOpacity onPress={onSkip} style={styles.skipBtn}>
          <Text style={[styles.skipText, { color: theme.muted }]}>Continue without signing in →</Text>
        </TouchableOpacity>
      </ScrollView>
