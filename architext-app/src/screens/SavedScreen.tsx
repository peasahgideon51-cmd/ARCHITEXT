...
export function SavedScreen() {
  ...
  // HomeScreen already knows how to rebuild a plan from `loadedEntry` (see
  // its useEffect watching route.params?.loadedEntry) — that's the same
  // mechanism the History screen uses. This just needed to actually be
  // wired up to a tap here.
  const openPlan = (item: SavedEntry) => {
    ...
    <View style={[styles.root, { backgroundColor: theme.pageBg }]}>
      <View style={styles.header}>
        ...
        <Text style={[styles.pageSub, { color: theme.muted }]}>
          Your bookmarked architectural designs.
        </Text>
      </View>
      ...
      {saved.length === 0 ? (
        ...
        >
          ...
          <Text style={[styles.emptyText, { color: theme.muted }]}>
            Save a floor plan from the builder to access it here anytime.
          </Text>
        </View>
      ) : (
        <FlatList
          ...
          renderItem={({ item }: { item: SavedEntry }) => (
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text
                  style={[styles.itemName, { color: theme.dark }]}
                  numberOfLines={1}
                >
                  {item.title}
                </Text>
                <Text style={[styles.metaText, { color: theme.muted }]}>
                  {item.rooms} rooms · {item.time}
                </Text>
              </View>
